...
// Redirect HOME so createRoutine / saveStore writes don't trample the
// user's real ~/.pi state file.
const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "pi-routines-drain-"));
